...
final notificationDummies = <TossNotification>[
  TossNotification(NotificationType.tossPay, '이번주에 영화 한편 어때요? CGV 할인 쿠폰이 도착했어요', DateTime.now().subtract(const Duration(minutes: 27)), isRead: true),
  TossNotification(NotificationType.stock, '인적분할에 대해 알려드릴게요.', DateTime.now().subtract(const Duration(hours: 1))),
  TossNotification(NotificationType.walk, "1,000걸음 이상 걸었다면 포인트 받으세요.", DateTime.now().subtract(Duration(hours: 1)), isRead: true),
  ...
  TossNotification(NotificationType.purchase, '띵동! 월요일 공동구매 보러가기', DateTime.now().subtract(Duration(hours: 12)), isRead: true)
];